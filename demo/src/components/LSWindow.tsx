...
export function LSWindow({ message, onDismiss }: LSWindowProps) {
  ...
  return (
    <div
      className={`fixed top-4 right-4 max-w-sm border rounded-lg p-4 shadow-lg z-50 ${getMessageTypeStyles(message.type)}`}
    >
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-2">
          <span className="text-lg">{messageTypeIcons[message.type]}</span>
          <div>
