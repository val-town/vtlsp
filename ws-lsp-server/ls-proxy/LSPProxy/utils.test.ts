...
describe("replaceFileUris", () => {
  ...
  it("should replace URIs in nested objects", () => {
    ...
    expect(convertedNested.resource.uri).toBe("converted:file:///path/to/resource.ts");
    expect(convertedNested.resource.specifier).toBe("converted:file:///specifier.ts");
  });
  ...
  it("should replace URIs in arrays", () => {
    ...
    expect(convertedArray.files[2].specifier).toBe("converted:file:///file3.ts");
  });
  ...
  it("should not replace URIs in non-URI keys", () => {
    ...
    expect(converted.description).toBe("This is a converted:file:///path/description.txt in a description");
    expect(converted.title).toBe("Contains converted:file:///example.js in title");
    expect(converted.content).toBe("Some converted:file:///content.md reference");
  });
  ...
  it("should handle mixed content with file URIs and other text", () => {
    const obj = {
      ...
      description: "Processing file:///data/input.csv and file:///config/settings.json",
      nonUriField: "This file:///path/should/be/converted.ts anyway",
    };
    const converted = replaceFileUris(obj, uriConverter) as any;

    expect(converted.message).toBe("Error in converted:file:///src/main.ts at line 42");
    expect(converted.uri).toBe("converted:file:///workspace/project.json");
    expect(converted.description).toBe("Processing converted:file:///data/input.csv and converted:file:///config/settings.json");
    expect(converted.nonUriField).toBe("This converted:file:///path/should/be/converted.ts anyway");
  });

  it("should handle primitive values", () => {
    expect(replaceFileUris("file:///test.ts", uriConverter)).toBe("converted:file:///test.ts");
    ...
    const complexObj = {
      project: {
        ...
        config: {
          ...
          dependencies: ["file:///node_modules/dependency1", "file:///node_modules/dependency2"],
        },
      },
    };
    const convertedComplex = replaceFileUris(complexObj, uriConverter) as any;

    expect(convertedComplex.project.files[0].uri).toBe("converted:file:///src/index.ts");
    expect(convertedComplex.project.files[1].uri).toBe("converted:file:///src/utils.ts");
    expect(convertedComplex.project.config.mainFile).toBe("converted:file:///src/index.ts");
    expect(convertedComplex.project.config.dependencies[0]).toBe("converted:file:///node_modules/dependency1");
    expect(convertedComplex.project.config.dependencies[1]).toBe("converted:file:///node_modules/dependency2");
  });
});