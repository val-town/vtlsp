...
export class LSPProxy {
  ...
  private async startProc() {
    ...
    this.process = spawn(
      this.#execOptions.command,
      this.#execOptions.args,
      {
        cwd: this.tempDir,
        env: {
          ...process.env,
    ...
    if (this.#lsLogStdoutPath) {
      this.process.stdout.pipe(fs.createWriteStream(this.#lsLogStdoutPath, { flags: "a" }));
    }

    if (this.#lsLogStderrPath) {
      this.process.stderr.pipe(fs.createWriteStream(this.#lsLogStderrPath, { flags: "a" }));
    }
  ...
  private setupProcToClient() {
    assert(this.procConn, "LSP connection not initialized");

    this.procConn.onNotification(async (method, params) => {
      logger.debug({ method, params }, "Received notification from process");

      const transformedParams = replaceFileUris(params, this.#uriConverters.fromProcUri);
      if (!isLspParamsLike(transformedParams)) throw new Error("Failed to transform params from process");
      ...
      if (handlerResult !== null) {
        if (hasALsProxyCode(handlerResult) && handlerResult.ls_proxy_code === "cancel_response") {
          return;
        }
        return handlerResult;
      }

      params = await this.applyMiddleware(method, params, this.#procToClientMiddlewares, false);
      logger.debug({ method, params }, "Transformed params from process");
      if (hasALsProxyCode(params) && params.ls_proxy_code === "cancel_response") return;
    ...
    this.procConn.onRequest(async (method, params) => {
      ...
      params = await this.applyMiddleware(method, params, this.#procToClientMiddlewares, true);
      logger.debug({ method, params }, "Transformed params from process");
      if (hasALsProxyCode(params) && params.ls_proxy_code === "cancel_response") return;
  ...
  private setupClientToProc() {
    this.clientConn.onNotification(async (method, params) => {
      ...
      const transformedParams = replaceFileUris(params, this.#uriConverters.toProcUri);
      if (!isLspParamsLike(transformedParams)) throw new Error("Failed to transform params from client");
      params = transformedParams;
      ...
      params = await this.applyMiddleware(method, params, this.#clientToProcMiddlewares, false);
    ...
    this.clientConn.onRequest(async (method, params) => {
      ...
      const transformedParams = replaceFileUris(params, this.#uriConverters.toProcUri); // params are from client, so we convert to proc URI
      if (!isLspParamsLike(transformedParams)) throw new Error("Failed to transform params from client, got: " + JSON.stringify(transformedParams));
      ...
      const transformedResp = replaceFileUris(resp, this.#uriConverters.fromProcUri); // response is from process, so we convert to client URI
      if (!isLspRespLike(transformedResp)) throw new Error("Failed to transform response from process, got: " + JSON.stringify(transformedResp));
      ...
      if (this.#procToClientMiddlewares[method as keyof LSPProxyProcToClientMiddlewares]) {
  ...
  private async applyMiddleware(
    method: string,
    params: any,
    middlewares: LSPProxyClientToProcMiddlewares | LSPProxyProcToClientMiddlewares,
    isRequest = false,
    ...
    if (exactMatch) {
      if (isClientToProc || !isRequest) {
        ...
        modifiedValue = await (exactMatch as ParamsMiddlewareFunction)(modifiedValue);
        logger.trace({ method, params, modifiedValue }, "Applied exact method middleware");
        if (modifiedValue.ls_proxy_code === "cancel_response") return modifiedValue;
      } else {
        // For proc-to-client requests, we modify result with original params as context
        modifiedValue = await (exactMatch as ResultMiddlewareFunction)(modifiedValue, params);
        logger.trace({ method, params, modifiedValue }, "Applied exact method middleware");
        if (modifiedValue.ls_proxy_code === "cancel_response") return modifiedValue;
      }
    }
    ...
    if (!isRequest && middlewares["notification/*"]) {
      const catchAll = middlewares["notification/*"] as CatchAllMiddlewareFunction;
      modifiedValue = await catchAll(method, params, result);
      logger.trace(
        { method, params, modifiedValue },
    ...
    if (middlewares["*"]) {
      ...
      modifiedValue = await catchAll(method, params, result);
      logger.trace({ method, params, modifiedValue }, "Applied catch-all middleware");
      if (modifiedValue.ls_proxy_code === "cancel_response") return modifiedValue;
    }
  ...
  >(
    ...
    if (exactMatch) {
      ...
      if (exactMatch.length <= 1) {
        ...
        logger.trace({ method, params, handlerResult }, "Applied exact method handler");
        if (hasALsProxyCode(handlerResult) && handlerResult.ls_proxy_code === "cancel_response") {
          return handlerResult;
        }
      } else {
        // Catch-all handler function (takes method and params)
        handlerResult = await (exactMatch as CatchAllHandlerFunction)(method, params);
        logger.trace({ method, params, handlerResult }, "Applied exact method catch-all handler");
        if (hasALsProxyCode(handlerResult) && handlerResult.ls_proxy_code === "cancel_response") {
          return handlerResult;
        }
      }
    ...
    if (isRequest && handlers["request/*"]) {
      ...
      logger.trace({ method, params, handlerResult }, "Applied request-specific catch-all handler");
      if (hasALsProxyCode(handlerResult) && handlerResult.ls_proxy_code === "cancel_response") {
        return handlerResult;
      }
    }
    ...
    if (handlers["*"]) {
      ...
      handlerResult = await catchAll(method, params);
      logger.trace({ method, params, handlerResult }, "Applied global catch-all handler");
      if (hasALsProxyCode(handlerResult) && handlerResult.ls_proxy_code === "cancel_response") {
        return handlerResult;
      }
    }
